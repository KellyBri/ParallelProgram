...
void __global__  sobel_Kernel (unsigned char *d_s,unsigned char *d_t,unsigned int width,unsigned int height,unsigned short byte_per_pixel) {
    ...
    if(y>=height) return;
		for (x = 0; x < width; ++x) {
				...
				for (v = -yBound; v < yBound + adjustY; ++v) {
					for (u = -xBound; u < xBound + adjustX; ++u) {
						if ((x + u) >= 0 && (x + u) < width && y + v >= 0 && y + v < height) {
							R = d_s[byte_per_pixel * (width * (y+v) + (x+u)) + 2];
							G = d_s[byte_per_pixel * (width * (y+v) + (x+u)) + 1];
							B = d_s[byte_per_pixel * (width * (y+v) + (x+u)) + 0];
			...
			d_t[byte_per_pixel * (width * y + x) + 2] = cR;
			d_t[byte_per_pixel * (width * y + x) + 1] = cG;
			d_t[byte_per_pixel * (width * y + x) + 0] = cB;
		}
...
int main(int argc, char **argv) {
    ...
    cudaMalloc( &d_s, (size_t) width * height * byte_per_pixel);
    cudaMalloc( &d_t, (size_t) width * height * byte_per_pixel);
    
    // Task 1: Memory copy from Host to Device (GPU)
    // Hint  : cudaMemcpy ( ... , cudaMemcpyHostToDevice )
    cudaMemcpy(d_s, host_s, (size_t) width * height * byte_per_pixel, cudaMemcpyHostToDevice);
    
    // Task 1: Modify sobel() to CUDA kernel function
    // Hint  : sobel_Kernel <<< ??? , ??? >>> ( ??? );
    sobel_Kernel<<<height/256+1,256>>>(d_s, d_t, width, height, byte_per_pixel);
    
    // Task 1: Memory Copy from Device (GPU) to Host
    // Hint  : cudaMemcpy ( ... , cudaMemcpyDeviceToHost )
    cudaMemcpy(host_t, d_t, (size_t) width * height * byte_per_pixel, cudaMemcpyDeviceToHost);
